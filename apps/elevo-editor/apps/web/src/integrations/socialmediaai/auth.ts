...
/**
 * Check URL params for auth tokens passed from the host Nuxt app (cross-origin in dev).
 * If found, store them in localStorage and strip the params from the URL.
 * Must be called before any auth checks.
 */
export function hydrateSessionFromUrl(): boolean {
	...
	// Strip auth tokens from the URL to keep it clean
