...
/** Build the full login URL on the host (Nuxt) app, with a redirect back to the current editor page. */
export function getHostLoginUrl(): string {
	const hostOrigin = getHostAppOrigin();
	const currentUrl = typeof window !== "undefined"
		? window.location.href
		: "";
	const loginPath = `/auth/login?redirect=${encodeURIComponent(currentUrl)}`;
	return hostOrigin ? `${hostOrigin}${loginPath}` : loginPath;
}
