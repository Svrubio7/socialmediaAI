...
import { socialMediaAiStorageAdapter } from "@/integrations/socialmediaai/storage-adapter";
import {
	migrations,
...
class StorageService {
	...
	constructor() {
		this.config = {
			projectsDb: "opencut-video-editor-projects",
			mediaDb: "opencut-video-editor-media",
			savedSoundsDb: "opencut-video-editor-saved-sounds",
			version: 1,
		};
	...
	async saveProject({ project }: { project: TProject }): Promise<void> {
		if (socialMediaAiStorageAdapter.isEnabled()) {
			await socialMediaAiStorageAdapter.saveProject(project);
			return;
		}
	...
	async loadProject({
		...
		if (socialMediaAiStorageAdapter.isEnabled()) {
			return socialMediaAiStorageAdapter.loadProject(id);
		}
	...
	async loadAllProjects(): Promise<TProject[]> {
		if (socialMediaAiStorageAdapter.isEnabled()) {
			const metadata =
				await socialMediaAiStorageAdapter.loadAllProjectsMetadata();
			const projects: TProject[] = [];

			for (const item of metadata) {
				const result = await socialMediaAiStorageAdapter.loadProject(item.id);
				if (result?.project) {
	...
	async loadAllProjectsMetadata(): Promise<TProjectMetadata[]> {
		if (socialMediaAiStorageAdapter.isEnabled()) {
			return socialMediaAiStorageAdapter.loadAllProjectsMetadata();
		}
	...
	async deleteProject({ id }: { id: string }): Promise<void> {
		if (socialMediaAiStorageAdapter.isEnabled()) {
			await socialMediaAiStorageAdapter.deleteProject(id);
			return;
		}
	...
	async saveMediaAsset({
		...
		if (socialMediaAiStorageAdapter.isEnabled()) {
			await socialMediaAiStorageAdapter.saveMediaAsset({ projectId, mediaAsset });
			return;
		}
	...
	async loadMediaAsset({
		...
		if (socialMediaAiStorageAdapter.isEnabled()) {
			const assets =
				await socialMediaAiStorageAdapter.loadAllMediaAssets({ projectId });
	...
	async loadAllMediaAssets({
		...
		if (socialMediaAiStorageAdapter.isEnabled()) {
			return socialMediaAiStorageAdapter.loadAllMediaAssets({ projectId });
		}
	...
	async deleteMediaAsset({
		...
		if (socialMediaAiStorageAdapter.isEnabled()) {
			await socialMediaAiStorageAdapter.deleteMediaAsset({ projectId, id });
			return;
		}
	...
	async deleteProjectMedia({
		...
		if (socialMediaAiStorageAdapter.isEnabled()) {
			await socialMediaAiStorageAdapter.deleteProjectMedia({ projectId });
			return;
		}
