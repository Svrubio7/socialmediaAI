<template>
  <header class="h-14 border-b border-surface-800 bg-surface-950/95 px-3 lg:px-4 backdrop-blur">
    <div class="h-full flex items-center justify-between gap-3">
      <div class="flex items-center gap-2 min-w-0">
        <button
          type="button"
          class="h-9 w-9 rounded-lg border border-surface-800 bg-surface-900 text-surface-400 hover:text-surface-100 hover:border-primary-400 transition-colors"
        ...
        <button
          type="button"
          class="inline-flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-surface-100 hover:bg-surface-900 transition-colors"
          @click="$emit('go-home')"
        >
          ...
          <span class="text-sm font-normal">Elevo<span class="text-primary-400">AI</span></span>
        </button>
        <input
      ...
      <div class="hidden md:flex items-center gap-1">
        <button
          type="button"
          class="h-8 w-8 rounded-md border border-surface-800 text-surface-400 transition-colors"
          :class="canUndoRef ? 'bg-surface-900 hover:text-surface-100 hover:border-primary-400' : 'bg-surface-950 opacity-50 cursor-not-allowed'"
          :disabled="!canUndoRef"
          @click="$emit('undo')"
          aria-label="Undo"
        >
        ...
        <button
          type="button"
          class="h-8 w-8 rounded-md border border-surface-800 text-surface-400 transition-colors"
          :class="canRedoRef ? 'bg-surface-900 hover:text-surface-100 hover:border-primary-400' : 'bg-surface-950 opacity-50 cursor-not-allowed'"
          :disabled="!canRedoRef"
          @click="$emit('redo')"
          aria-label="Redo"
        >
      ...
      <div class="flex items-center gap-2">
        <button
          type="button"
          class="inline-flex items-center gap-1.5 rounded-md bg-primary-500 px-3 py-1.5 text-sm font-normal text-surface-50 hover:bg-primary-400 transition-colors"
          @click="$emit('export')"
        >
        ...
        <button
          type="button"
          class="hidden sm:flex h-8 w-8 items-center justify-center rounded-md border border-surface-800 bg-surface-900 text-surface-400 hover:text-surface-100 hover:border-primary-400 transition-colors"
          @click="$emit('help')"
          aria-label="Help"
        >
        ...
        <button
          type="button"
          class="h-8 w-8 rounded-full bg-surface-800 border border-surface-700 text-sm font-normal text-surface-100"
          @click="$emit('account')"
        >
          {{ accountInitialRef }}
        </button>
