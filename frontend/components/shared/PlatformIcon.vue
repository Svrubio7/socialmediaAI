...
const colorClasses = computed(() => {
  const colors = {
    instagram: props.variant === 'filled' 
      ? 'bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400 text-white' 
      : 'border border-pink-500/30 text-pink-400',
    tiktok: props.variant === 'filled'
      ? 'bg-surface-900 text-white'
      : 'border border-surface-700 text-surface-300',
    youtube: props.variant === 'filled'
      ? 'bg-red-600 text-white'
      : 'border border-red-500/30 text-red-400',
    facebook: props.variant === 'filled'
